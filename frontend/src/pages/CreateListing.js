...
const CreateListing = () => {
  ...
  // const apiKey = process.env.MAPS_APIKEY;
...
useEffect(() => {
  ...
  const onSubmitHandler = async(e) => {
    ...
    let geoLocation ={};
    const response = await fetch(`https://geocode.search.hereapi.com/v1/geocode?q=${address}&apiKey=quhVrdb2B-bvrDCtO1tp14k3VKC4-6nGCh9BuZUBQTA`)
    ...
    try {
      ...
      const data = await request(`/property`, 'POST', options, {...formData, img:filenames, latitude:geoLocation.lat, longitude:geoLocation.lng})
      if(data){
